...
data:
  ...
  bundledCode: "#line 2 \"include/mtl/bit_manip.hpp\"\n#include <cstdint>\n#include\
    ...
    \ likes std::vector<bool>\r\nstruct Bitmap {\r\n  using value_type = bool;\r\n\
    \  using W = uint64_t;\r\n  std::vector<W> arr;\r\n  size_t sz;\r\n  static constexpr\
    \ size_t required_word_size(size_t n) {\r\n    return (n+63) / 64;\r\n  }\r\n\
    \  static constexpr W word_filled_by(bool bit) {\r\n    return bit ? 0xFFFFFFFFFFFFFFFF\
    \ : 0;\r\n  }\r\n  explicit Bitmap(size_t n = 0, bool bit = false) : arr(required_word_size(n),\
    \ word_filled_by(bit)), sz(n) {}\r\n  template<typename It>\r\n  Bitmap(It begin,\
    \ It end) : sz(0) {\r\n    using trait = std::iterator_traits<It>;\r\n    using\
    \ iterator_category = typename trait::iterator_category;\r\n    static_assert(std::is_base_of<std::input_iterator_tag,\
    ...
    \     qs.pop();\n    }\n    return ret;\n  }\n};\n#line 9 \"include/mtl/ordinal_range_search.hpp\"\
    \n\r\n/**\r\n * @brief Ordinal Range Search\r\n *   Maintain 2-dimential points\
    \ (x,y) and their weights.\r\n*/\r\ntemplate<typename T, typename W = size_t,\
    \ \r\n  T IndexMax = std::numeric_limits<T>::max(), \r\n  class BVec = BitVector\
    \ // TODO: optimize size of succinct BV which size of B(n,u)+O(n) bit\r\n  >\r\
    \nstruct ORS {\r\n  std::vector<std::tuple<T,T,W>> ps;\r\n  WaveletMatrix<int>\
    \ wm;\r\n  FenwickTree<W> rsq;\r\n  using bit_vector_type = BVec;\r\n  bit_vector_type\
    \ X, Y;\r\n  T value_of_ith_x(size_t i) const {\r\n    return X.template select<1>(i)\
    \ - i;\r\n  }\r\n  size_t index_of_lower_bound_x(const T& x) const {\r\n    return\
    \ x==0 ? 0 : (X.template select<0>(x-1) - (x - 1));\r\n  }\r\n  std::pair<size_t,\
    \ size_t> range_on_wm_lower_bound_x(const T& x) const {\r\n    auto l = x==0 ?\
    \ 0 : (X.template select<0>(x-1) - (x - 1));\r\n    auto lbx = value_of_ith_x(l);\r\
    \n    auto r = X.template select<0>(lbx) - lbx;\r\n    return std::make_pair(l,\
    \ r);\r\n  }\r\n  T value_of_ith_y(size_t i) const {\r\n    return Y.template\
    \ select<1>(i);\r\n  }\r\n  size_t index_of_lower_bound_y(const T& y) const {\r\
    \n    return Y.rank(y);\r\n  }\r\n\r\n  ORS() = default;\r\n\r\n  void add(T x,\
    \ T y, W w = 1) {\r\n    ps.emplace_back(x,y,w);\r\n  }\r\n  void build() {\r\n\
    \    std::sort(ps.begin(), ps.end(),\r\n              [](auto l, auto r) {\r\n\
    \      return std::get<0>(l) != std::get<0>(r) ? std::get<0>(l) < std::get<0>(r)\r\
    \n                                : std::get<1>(l) < std::get<1>(r);\r\n    });\r\
    \n\r\n    {\r\n      std::vector<T> xs(ps.size()+1), ys(ps.size()+1);\r\n    \
    \  for (size_t i = 0; i < ps.size(); i++) {\r\n        xs[i] = std::get<0>(ps[i]);\r\
    \n        ys[i] = std::get<1>(ps[i]);\r\n      }\r\n      xs.back() = IndexMax;\r\
    \n      X = bit_vector_type((size_t)IndexMax + xs.size());\r\n      for (size_t\
    \ i = 0; i < xs.size(); i++)\r\n        X[xs[i] + i] = 1;\r\n      X.build();\r\
    \n\r\n      ys.back() = IndexMax;\r\n      std::sort(ys.begin(), ys.end());\r\n\
    \      ys.erase(std::unique(ys.begin(), ys.end()), ys.end());\r\n      Y = bit_vector_type((size_t)IndexMax\
    \ + 1);\r\n      for (auto y:ys) Y[y] = 1;\r\n      Y.build();\r\n    }\r\n\r\n\
    \    {\r\n      std::vector<int> S(ps.size());\r\n      std::vector<std::pair<int,\
    \ W>> SW(ps.size()), swz, swo;\r\n      for (size_t i = 0; i < ps.size(); i++)\
  ...
  code: "#pragma once\r\n#include \"fenwick_tree.hpp\"\r\n#include \"succinct/wavelet_matrix.hpp\"\
    ...
    #include <cstddef>\r\n#include <limits>\r\n#include <vector>\r\n\r\n/**\r\n *\
    \ @brief Ordinal Range Search\r\n *   Maintain 2-dimential points (x,y) and their\
    \ weights.\r\n*/\r\ntemplate<typename T, typename W = size_t, \r\n  T IndexMax\
    \ = std::numeric_limits<T>::max(), \r\n  class BVec = BitVector // TODO: optimize\
    \ size of succinct BV which size of B(n,u)+O(n) bit\r\n  >\r\nstruct ORS {\r\n\
    \  std::vector<std::tuple<T,T,W>> ps;\r\n  WaveletMatrix<int> wm;\r\n  FenwickTree<W>\
    \ rsq;\r\n  using bit_vector_type = BVec;\r\n  bit_vector_type X, Y;\r\n  T value_of_ith_x(size_t\
    \ i) const {\r\n    return X.template select<1>(i) - i;\r\n  }\r\n  size_t index_of_lower_bound_x(const\
    \ T& x) const {\r\n    return x==0 ? 0 : (X.template select<0>(x-1) - (x - 1));\r\
    \n  }\r\n  std::pair<size_t, size_t> range_on_wm_lower_bound_x(const T& x) const\
    \ {\r\n    auto l = x==0 ? 0 : (X.template select<0>(x-1) - (x - 1));\r\n    auto\
    \ lbx = value_of_ith_x(l);\r\n    auto r = X.template select<0>(lbx) - lbx;\r\n\
    \    return std::make_pair(l, r);\r\n  }\r\n  T value_of_ith_y(size_t i) const\
    \ {\r\n    return Y.template select<1>(i);\r\n  }\r\n  size_t index_of_lower_bound_y(const\
    \ T& y) const {\r\n    return Y.rank(y);\r\n  }\r\n\r\n  ORS() = default;\r\n\r\
    \n  void add(T x, T y, W w = 1) {\r\n    ps.emplace_back(x,y,w);\r\n  }\r\n  void\
    \ build() {\r\n    std::sort(ps.begin(), ps.end(),\r\n              [](auto l,\
    \ auto r) {\r\n      return std::get<0>(l) != std::get<0>(r) ? std::get<0>(l)\
    \ < std::get<0>(r)\r\n                                : std::get<1>(l) < std::get<1>(r);\r\
    \n    });\r\n\r\n    {\r\n      std::vector<T> xs(ps.size()+1), ys(ps.size()+1);\r\
    \n      for (size_t i = 0; i < ps.size(); i++) {\r\n        xs[i] = std::get<0>(ps[i]);\r\
    \n        ys[i] = std::get<1>(ps[i]);\r\n      }\r\n      xs.back() = IndexMax;\r\
    \n      X = bit_vector_type((size_t)IndexMax + xs.size());\r\n      for (size_t\
    \ i = 0; i < xs.size(); i++)\r\n        X[xs[i] + i] = 1;\r\n      X.build();\r\
